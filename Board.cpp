...
void CBoard::DrawGameOver()
{
    ...
    DrawTextW(GET_HDC(), L"R: 재시작   ESC: 로비로", -1, &hintRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
}
...
void CBoard::DrawWinner()
{
    ...
    DrawTextW(GET_HDC(), L"ESC: 로비로", -1, &hintRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
}
...
void CBoard::DrawRestartVotes()
{
    ...
    wchar_t buf[32];
    swprintf_s(buf, L"재시작 대기: %d/%d", m_nRestartVotes, m_nRestartTotal);
...
void CBoard::DrawReplayInfo()
{
    ...
    wchar_t buf[32];
    if (m_bReplayPaused)
        swprintf_s(buf, L"상태: 일시정지");
    else
        swprintf_s(buf, L"배속: x%.1f", m_fReplaySpeed);
